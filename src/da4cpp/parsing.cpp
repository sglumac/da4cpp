...
std::string get_symbol_name(const CXCursor &cursor) {
...
Symbol type_declaration_from_cursor(const CXCursor &cursor) {
  ...
  const SymbolType type = isDefinition
                          ? SymbolType::TypeDefinition
                          : SymbolType::TypeDeclaration;
  return from_cursor(cursor, type);
}

Symbol type_definition_from_cursor(const CXCursor &cursor) {
  return from_cursor(cursor, SymbolType::TypeDefinition);
}
...
bool is_function_reference(const CXCursor &cursor) {
  ...
  return (cursorKind == CXCursor_DeclRefExpr || cursorKind == CXCursor_CallExpr)
    && (clang_Cursor_isNull(referenced) == 0)
